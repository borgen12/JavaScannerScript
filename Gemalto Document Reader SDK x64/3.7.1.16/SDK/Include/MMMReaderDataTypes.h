...
//  exclusively used by Hardware AutoCapture setup - not a real light code for the lowlevelapi
#define	LIGHT_BARCODE	256
...
/// parameter to the MMMReader_SetLoggingOptions() and MMMReader_GetLoggingOptions() function.
///
...
typedef struct LoggingSettings
{
	...
	/// If #loggingStrategy is set to #MRLS_FILE_SIZE, specify the
	/// maximum number of files to keep. When the current log file
	/// reaches or exceeds this limit, it will be closed and the first log file
	/// is overwritten.  Logging is picked up at the most recently modified file.
	unsigned int maxLogFiles;

	/// If true, log dates are month / day / year 
	MMMR_BOOL monthDayYearOrder;
} LoggingSettings;
...
#define	LOGMASK_IDVREMOTE	0x00000080ul	///< Log mask value to log IDV Remote related messages.
/// \endcond
// PAGESDK
...
{	
	...
	/// Signal to state that document processing is complete, before success/failure and end-of-doc.
	SIGNAL_PROCESSING_COMPLETE,

	/// Signal to turn LEDs OFF.
	SIGNAL_LED_OFF,
...
// RF issues
	...
	/// \ifnot SWIPESDK
	/// Failed to decode the EF.ChipSecurity file from the RF chip.
	/// \else
	/// \attention This error code is not used by the \sdk
	/// \endif
    ERROR_RF_DECODE_EF_CHIP_SECURITY_FAILURE,

	/// \ifnot SWIPESDK
	/// Failed to decode the EF.CardSecurity file from the RF chip.
	/// \else
	/// \attention This error code is not used by the \sdk
	/// \endif
    ERROR_RF_DECODE_EF_CARD_SECURITY_FAILURE,

	/// \ifnot SWIPESDK
	/// Failed to decode the EF.CardAccess file from the RF chip.
	/// \else
	/// \attention This error code is not used by the \sdk
	/// \endif
    ERROR_RF_DECODE_EF_CARD_ACCESS_FAILURE,
...
//#endif //DCA
	...
	ERROR_FACE_MATCH_INVALID_PRODUCT_KEY,		///< Cognet Face Match - FaceMatching_InvalidProductKey.
	ERROR_FACE_MATCH_PRODUCT_KEY_EXPIRED,		///< Cognet Face Match - FaceMatching_ProductKeyExpired.
	ERROR_FACE_MATCH_GENERAL_FAILURE,			///< Cognet Face Match - General Failure.
...
typedef enum ReaderState 
{
	...
	/// images to the sdk from an external source (other than a physical reader)
	///
...
typedef enum MMMReaderSoundMode
{
	...
	/// Use this value to generate beeps via the buzzer on equipped devices, such as the AT10Ki reader.
	/// The same as SM_BEEP, but on the reader instead of the host computer.
	/// Defaults to SM_BEEP if reader does not support beeps.
	SM_READER_BEEP
...
#define ROTATE_90			(0x0001)
#define ROTATE_180			(0x0002)
#define ROTATE_270			(0x0004)
#define ROTATE_ALL			(0x0008)
...
typedef enum AllowedPasswordMethods
{
	ALLOWED_METHOD_MRZ,
	ALLOWED_METHOD_MRZ_CAN,
	ALLOWED_METHOD_PIN,
	ALLOWED_METHOD_PIN_CAN,
	ALLOWED_METHOD_PIN_CAN_PUK
} AllowedPasswordMethods;
...
typedef struct RFAccessControlPasswords
{
	...
	RTCHAR puPIN[10];	// 6 digit like CAN
	RTCHAR puPUK[10];	// 10 digit PUK
...
typedef bool (*MMMReaderRFPasswordCallback) (
	...
	AllowedPasswordMethods aAllowedPasswordMethods,
...
/// Defines reader detection states for document readers.
///
typedef enum ReaderDocumentDetectionState
{
	NoDocument = 0,
	MovingDocument = 1,
	FoundDocument = 2
} ReaderDocumentDetectionState;

typedef struct ReaderDocumentDetection 
{
	ReaderDocumentDetectionState state;
	void *image;
	int imageLength;
} ReaderDocumentDetection;


/// Callback function definition for a user-defined function that will receive event 
/// notifications of document detection that are raised by the \sdk.
///
/// \ingroup EventsModule
///
/// \param[in] aParam An optional pointer to some user-defined data. This is usually initialised
/// when setting the callback function in question.
///
/// \param[in] aReaderDocumentDetection Specifies the current document detection state and option image by the \sdk. See 
/// #ReaderDocumentDetectionState for the kinds of states that can be sent.
///
/// \remarks \a aParam can be used to provide some context data back through the callback. One
/// example could be the \c this pointer from a C++ class; in this case, because the callback
/// function must be a C-style function, \a aParam would be able to access elements of its
/// class type.
typedef void(*MMMReaderDetectionStatusCallback) (
	void *aParam,
	ReaderDocumentDetection aReaderDocumentDetection
);
...
typedef void (*MMMReaderErrorCallback) (
	enum MMMReaderErrorCode aErrorCode,
	RTCHAR *aErrorMsg,
	void *aParam
);
...
/// and improve document identification.
///
...
typedef enum CERT_TYPE
{
    ...
    CT_AT_CERT = 8,							///< An Authentication Terminal (AT) certificate.
	CT_DOC_SIGNER_CERT_DGC = 9				///< A DSC for a Digital Green Certificate (DGC).
} CERT_TYPE;

/// Defines the applications that can be read during RF operations.
///
/// \ingroup RfidModule
typedef enum PASSPORT_APPLICATION
{
    EPASSPORT,
    EID,
	EDL,
...
typedef enum DOC_SIGNER_CERT_MODE
{
    ...
    /// Load the DSC from the RF chip and verify it using a Inspection System Server.
...
/// when setting the callback function in question.
/// 
/// \param[in] aCertIdentifier A string or binary containing an identifier for the required certificate.
///
...
/// a DER encoded certificate (this can also be Base64 DER encoded).</td></tr>
///
/// <tr><td>#CT_AT_CERT \eac</td><td>The \sdk is requesting that an external Authentication Terminal 
/// (AT) certificate must be provided. This request will contain a value to match with the 
/// Certificate Authority Reference (CAR) field in the CV certificate. The function must supply 
/// a DER encoded certificate (this can also be Base64 DER encoded).</td></tr>
///
...
///
/// <tr><td>#CT_DOC_SIGNER_CERT_DGC</td><td>The \sdk is requesting that an external Document 
///	Signer Certificate (DSC) for an EU Digital Green Certificate/Digital COVID Certificate must be provided. 
/// This request contains the KID from the CBOR Web Token used to lookup the DSC.  KID is the first 
/// 8 bytes of the sha256 hash of the certificate. 
/// See <a href="https://ec.europa.eu/health/sites/default/files/ehealth/docs/digital-green-certificates_v3_en.pdf" target="_blank">DGC/DCC Specification</a></td></tr>
///
///	</table>
...
typedef enum MMMReaderTerminalType
{
	INSPECTION_SYSTEM,
	AUTHENTICATION_TERMINAL,
	SIGNATURE_TERMINAL
} MMMReaderTerminalType;

typedef enum RF_APPLICATION_MODE
{
	// Read ePassport
	APP_EPASSPORT = 1,
...
//typedef bool (*MMMReaderTerminalAccessRightsCallback)(
//    void* aParam,
//    char* aBufferToSign, 
//	int aBufferToSignLen,
//    char* aCertBuffer,
//	int aCertBufferLen,
//	MMMReaderTerminalType aTerminalType,
//    char* aSignature, 
//	int* aSignatureLen
//);
...
typedef enum MMMReaderRFItem
{
	...
	RFID_EF_CARD_SECURITY,			///< The EF.CardSecurity file
	RFID_EF_CHIP_SECURITY,			///< The EF.ChipSecurity file

	NUM_RF_ITEMS					///< \internal
} MMMReaderRFItem;

/// Defines the status items that can be retrieved from an RF chip.
///
/// \ingroup RfidModule
///
/// \sa MMMReader_RFGetStatus()
typedef enum MMMReaderRFStatus
{
	RFID_AGE_VERIFICATION_STATUS,
	RFID_DOCUMENT_VALIDITY_VERIFICATION_STATUS,
	RFID_COMMUNITY_ID_VERIFICATION_STATUS,
	
	RFID_RESTRICTED_IDENTIFICATION_STATUS,
...
typedef enum MMMPluginID
{
	...
	MMM_PID_China_Entry_Exit_Stamp = 21
} MMMPluginID;
...
typedef struct SmartCardReaderInfo
{
	...
	/// Provides the fimrware version of the smart card reader, if available.
	RTCHAR puFirmwareVersion[MAX_READERNAME_LEN];
...
typedef struct MMMReaderHardwareConfig
{
	...
	/// Indicates whether the scanner has limited power (milliAmps) available.  This usually happens when 
	/// a scanner being powered exclusively via USB 2 cable and no external power supply. This member is exclusively 
	/// used to control parallel or sequenced functioning of the optical and rf modules on the reader. Note that for some 
	/// scanners that can work either USB powered or externally powered, this could change on successive calls
	/// if the user has unplugged the scanner.
	bool puIsUSBPowered;

	/// Indicates whether the scanner has an external power supply. Note that for some 
	/// scanners that can work either USB powered or externally powered, this could change on successive calls
	/// if the user has unplugged the scanner.
	bool puIsExternallyPowered;
	...
	/// Indicates column shift in pixels to perform on debarrel (post debarrel shift)
	int puDebarrelColumnShift;

	/// Indicates row shift in pixels to perform on debarrel (post debarrel shift) 
	int puDebarrelRowShift;

	/// 21 may 2019 - new feature on PCB boards - is not available on old PCB boards
	/// Indicates the reader PCB revision number reported via the firmware interface 
	long puPCBRevision;
...
typedef enum MMMSwipeDeviceType
{
	...
	/// The CR series of document readers.
	MSDT_MMM_CR_SERIES,
...
typedef struct MMMReaderCodelineData
{
	...
	/// Provides the date of birth of the document owner as printed on the document 
	char DateOfBirthMRZ[MAX_CODELINE_FIELD_LENGTH];

	/// Provides the expiry date of the document owner as printed on the document
	char ExpiryDateMRZ[MAX_CODELINE_FIELD_LENGTH];
...
typedef struct QaCodelineMeasurements
{
	...
	QALineResults puLineData[MAXLINES];
...
/// Defines data about the photo found in a document.
///
/// \note This structure is only relevant if using the \hlapi.
///
/// \sa #MMMReaderDataType::CD_IMAGEPHOTODATA
typedef struct MMMReaderPhotoData
{
	/// Indicates the photo was found on the rear side image - when two sided reader is used (ie CR5400) 
	bool puOnRearSide;

	/// Specifies the location of the face, if any, within the relative front/rear image.
	Box puFaceLocation;

	/// Specifies the location the photo was cropped to within the relative front/rear image.
	Box puPhotoCropLocation;
...
typedef struct QaCompareMeasurements
{
	...
	float AverageContrastMin;
	float AverageContrastMax;
	...
	QaCompareMeasurementsLine LineData[MAXLINES];
...
typedef struct MMMReaderProcessResultTextZone
{
	...
	///String/Date data item(s) for each text field defined in the layout database of the document
...
{	
	...
	///A flag to indicate if the SDK can be used without a reader attached
	bool Readerless;
...
#define EIDAS_DATE_FIELD_LENGTH			9

// Character Country Code (1-3 bytes)
#define EIDAS_COUNTRY_FIELD_LENGTH		4

#define EIDAS_STREET_FIELD_LENGTH		100
#define EIDAS_CITY_FIELD_LENGTH			50
#define EIDAS_STATE_FIELD_LENGTH		50
#define EIDAS_ZIPCODE_FIELD_LENGTH		20
#define EIDAS_MAX_TEXT_LENGTH			200
#define EIDAS_MAX_RESIDENCE_COUNT		10

typedef struct _MMMReaderPlace {
	char Street[EIDAS_STREET_FIELD_LENGTH];
	char City[EIDAS_CITY_FIELD_LENGTH];
	char State[EIDAS_STATE_FIELD_LENGTH];
	char Country[EIDAS_COUNTRY_FIELD_LENGTH];
	char ZipCode[EIDAS_ZIPCODE_FIELD_LENGTH];
} MMMReaderPlace;

typedef struct _MMMReaderPeriodRange {
	char ExpiryDate[EIDAS_DATE_FIELD_LENGTH];
	char IssuanceDate[EIDAS_DATE_FIELD_LENGTH];
} MMMReaderPeriodRange;

#define PERIOD_CHOICE_NONE			0
#define PERIOD_CHOICE_EXPIRY		1
#define PERIOD_CHOICE_RANGE			2

typedef struct _MMMReaderPeriod {
	int PeriodChoice;
	struct {
		char ExpiryDate[EIDAS_DATE_FIELD_LENGTH];
		MMMReaderPeriodRange PeriodRange;
	} Choice;
} MMMReaderPeriod, MMMReaderValidityPeriod;

#define ENTITY_CHOICE_NONE			0
#define ENTITY_CHOICE_COUNTRY		1
#define ENTITY_CHOICE_PLACE			2

typedef struct _MMMReaderIssuingEntity {
	int EntityChoice;
	struct {
		char IssuingState[EIDAS_STATE_FIELD_LENGTH];
		MMMReaderPlace IssuingPlace;
	} Choice;
} MMMReaderIssuingEntity;

#define GENERAL_PLACE_CHOICE_NONE			0
#define GENERAL_PLACE_CHOICE_STRUCTURED		1
#define GENERAL_PLACE_CHOICE_FREE_TEXT		2
#define GENERAL_PLACE_CHOICE_NO_PLACE_INFO	3

typedef struct _MMMReaderGeneralPlace {
	int GeneralPlaceChoice;
	struct {
		MMMReaderPlace StructuredPlace;
		char FreeTextPlace[EIDAS_MAX_TEXT_LENGTH];
		char NoPlaceInfo[EIDAS_MAX_TEXT_LENGTH];
	} Choice;
} MMMReaderGeneralPlace, MMMReaderPlaceOfBirth;

typedef struct _MMMReaderMultiplePlaces {
	int Count;
	MMMReaderGeneralPlace Residences[EIDAS_MAX_RESIDENCE_COUNT];
} MMMReaderMultiplePlaces;

#define PLACE_OF_RESIDENCE_CHOICE_NONE						0
#define PLACE_OF_RESIDENCE_CHOICE_RESIDENCE					1
#define PLACE_OF_RESIDENCE_CHOICE_MULTIPLE_RESIDENCES		2

typedef struct _MMMReaderPlaceOfResidence{
	int ResidenceChoice;
	struct {
		MMMReaderGeneralPlace Residence;
		MMMReaderMultiplePlaces MultipleResidences;
	} Choice;
} MMMReaderPlaceOfResidence;

/// Defines the fields used for eID Auxiliary Data Verification.
///
/// \ingroup RfidModule
typedef struct eIDAuxiliaryData
{
	/// Provides the date used for Age Verification with the RF application.
	MMMReaderDate AgeVerificationDate;

	/// Provides the date used for Document Validity Verification with the application, typically the current date.
	MMMReaderDate DocumentValidationDate;

	/// Provides the Community ID for verification with the RF application.
	///
	/// \remarks Hex String Ex. "02761100000000"
	char CommunityID[25];

} eIDAuxiliaryData;

/// Defines the fields used for eID Restricted Identification.
///
/// \ingroup RfidModule
typedef struct eIDRestrictedIdentificationData
{
	/// Sector Public Key 1 Data Object starting with 0x7F49, per BSI 03110 Part 3 B.4.1 and Appendix D.3
	unsigned char SectorPK1[1024];

	/// Sector Public Key 1 Length.
	int SectorPK1Len;

	/// Sector Public Key 2 Data Object starting with 0x7F49, per BSI 03110 Part 3 B.4.1 and Appendix D.3
	unsigned char SectorPK2[1024];

	/// Sector Public Key 2 Length.
	int SectorPK2Len;

	/// Sector Identifier 1 returned by chip corresponding to SectorPK1.
	unsigned char SectorIdentifier1[100];

	/// Sector Identifier 1 Length.
	unsigned int SectorIdentifier1Len;

	/// Sector Identifier 2 returned by chip corresponding to SectorPK2.
	unsigned char SectorIdentifier2[100];

	/// Sector Identifier 2 Length.
	unsigned int SectorIdentifier2Len;
} eIDRestrictedIdentificationData;

//-------------------------------------------------------------------------------------------

#define EDL_MAX_TEXT_LENGTH				200
#define EDL_MAX_CATEGORY_TEXT_LENGTH	10
#define EDL_MAX_VEHICLE_CATEGORIES		50

typedef struct eDLVehicleCategory
{
	char Category[EDL_MAX_CATEGORY_TEXT_LENGTH];
	MMMReaderDate DateOfIssue;
	MMMReaderDate DateOfExpiry;
	char Code[EDL_MAX_CATEGORY_TEXT_LENGTH];
	char Sign[EDL_MAX_CATEGORY_TEXT_LENGTH];
	char Value[EDL_MAX_TEXT_LENGTH];
} eDLVehicleCategory;

/// Defines the fields used for eDLDataGroup1Data parsing.
///
/// \ingroup RfidModule
typedef struct eDLDataGroup1Data
{
	char TypeApprovalNumber[EDL_MAX_TEXT_LENGTH];
	char IssuingCountry[4];
	char FamilyName[EDL_MAX_TEXT_LENGTH];
	char GivenNames[EDL_MAX_TEXT_LENGTH];
	MMMReaderDate DateOfBirth;
	char PlaceOfBirth[EDL_MAX_TEXT_LENGTH];
	char Nationality[4];
	char Gender;
	MMMReaderDate DateOfIssue;
	MMMReaderDate DateOfExpiry;
	char IssuingAuthority[EDL_MAX_TEXT_LENGTH];
	char AdministrativeNumber[EDL_MAX_TEXT_LENGTH];
	char LicenseNumber[EDL_MAX_TEXT_LENGTH];
	char NormalPlaceOfResidence[EDL_MAX_TEXT_LENGTH];
	int VehicleCategoryCount;
	eDLVehicleCategory VehicleCategories[EDL_MAX_VEHICLE_CATEGORIES];
} eDLDataGroup1Data;

/// Defines reader provisioning states for iSeries (intelligent) readers.
///
typedef enum ReaderProvisioningState
{
	Manufacturing = 0,
	UnProvisioned = 1,
	Provisioned = 2,
	Updating = 3,
	ProvisioningSetup = 4,
	ProvisionedLegacyUsb = 5
} ReaderProvisioningState;

/// Defines reader connection states for iSeries (intelligent) readers.
///
typedef enum ReaderConnectionState
{
	NotConnected = 0,
	Connected = 1,
	Session = 2
...
/// Defines lite data capture results.
///

#define FieldNameMaxLength	50
#define FieldValueMaxLength	200
#define DataFieldsMaxLength	20

typedef struct LiteDataField
{
	char FieldName[FieldNameMaxLength];
	wchar_t FieldValue[FieldValueMaxLength];
} LiteDataField;

typedef struct LiteDataCapture
{
	float IdentificationConfidence ;
	char DocumentName[FieldValueMaxLength];
	char DocumentType[FieldValueMaxLength];
	bool FoundToBeRotated180;
	bool FoundtoBePortrait;
	bool FoundtoBeBackSide;
	// 
	int DataFieldCount;
	LiteDataField DataFields[DataFieldsMaxLength];
	//
	bool PhotoAvailable;
	int PhotoLocationLeft;
	int PhotoLocationRight;
	int PhotoLocationTop;
	int PhotoLocationBottom;
} LiteDataCapture;


typedef enum FieldDataType
{
	DocumentTypeFieldDataType,
	DocumentNumberFieldDataType,
	SurnameFieldDataType,
	GivenFieldDataType,
	GenderFieldDataType,
	BirthDateFieldDataType,
	IssueDateFieldDataType,
	ExpiryDateFieldDataType,
	PlaceOfBirthFieldDataType,
	NationalityFieldDataType,
	IssuingAuthorityFieldDataType,
	ReferenceNumberFieldDataType,
	AddressField1DataType,
	AddressField2DataType,
	AddressField3DataType,
	AddressField4DataType,
	NumberOfFieldDataType
} FieldDataType;

#define MAX_LETTER_COUNT	100
#define MAX_FIELD_LIST	20

typedef struct ExtractedFieldEntry
{
	FieldDataType fieldType;
	wchar_t letters[MAX_LETTER_COUNT];
} ExtractedFieldEntry;

typedef struct TextDataExtracted
{
	ExtractedFieldEntry fieldList[MAX_FIELD_LIST];
	int fieldListCount;
} TextDataExtracted;
