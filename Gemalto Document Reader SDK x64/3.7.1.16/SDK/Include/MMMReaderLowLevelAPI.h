...
MMM_READER_LOW_LEVEL_API MMMReaderHardwareConfig *MMMReader_GetHardwareConfig(bool aUpdateUsbStatusMembers = false);
...
/// \internal
MMM_READER_LOW_LEVEL_API unsigned char* MMMReader_ReadProdSetupInfo(int* aReadLen);
MMM_READER_LOW_LEVEL_API bool MMMReader_ConnectUSBHost(int connectTo);

MMM_READER_LOW_LEVEL_API bool MMMReader_ReadMACAddress(unsigned int *aOptions, unsigned char *aEthernetMAC, unsigned char *aWIFIMAC);
MMM_READER_LOW_LEVEL_API bool MMMReader_WriteMACAddress(unsigned int aOptions, unsigned char *aEthernetMAC, unsigned char *aWIFIMAC);
MMM_READER_LOW_LEVEL_API bool MMMReader_EraseMACAddress(void);
MMM_READER_LOW_LEVEL_API void MMMReader_SetProvisionedState(ReaderProvisioningState aProvisioningState);
MMM_READER_LOW_LEVEL_API void MMMReader_SetConnectionState(ReaderConnectionState aConnectionState);
MMM_READER_LOW_LEVEL_API ReaderProvisioningState MMMReader_GetProvisionedState();
/// \endcond
// PAGESDK
...
/// \internal
/// Does an Initialize and Shutdown to ensure leds are in power ready state.  For CBP where they are killing off sibling threads
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_CameraResetStatusLeds(CAMERASET *aCameraSettings);
/// \endcond
...
///
/// \param[in] aDetectionStatusCallback A user-defined
/// #MMMReaderDetectionStatusCallback to receive state notifications during document detection
/// Can be \c NULL.
///
/// \param[in] aDetectImage Enable or Disable sending the image used for document detection in the aDetectionStatusCallback.
///
...
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_CameraDetectDocument(
	...
	MMMReaderDetectionStatusCallback aDetectionStatusCallback,
	bool aDetectImage,
	void *aParam,
	HANDLE aEvent,
...
/// Crops an image to display only the face/photo region of a document image by
...
/// point to the cropped image result.
///
/// \param[in] aTryAllRotationsNeeded A boolean indicating whether to try finding a face
/// in all aRotationsNeeded or to stop when the first face is found in aRotationsNeeded.
/// The biggest face is used if true and a face is found in multiple rotations.
///
/// \param[in] aRotationsNeeded An int specifying which rotations of aInputImage to
/// try finding a face. No rotation (0) is always attempted. Use ROTATE_90, ROTATE_180, ROTATE_270 or ROTATE_ALL
/// bitwise as needed.
///
/// \param[out] aRotationFound A pointer to an int which will be updated to the rotation
/// of the input image in which the face was found. (0, 90, 180, 270)
///
/// \param[out] aFaceLocation A pointer to a Box which will be updated to the location of
/// the face found in the input image.  NULL if not desired.
///
/// \param[out] aCropLocation A pointer to a Box which will be updated to the location of
/// the cropped photo in the input image. Only set if aCropToPhoto = true.
/// NULL if not desired.
///
/// \param[out] aFaceLocationRotated A pointer to a Box which will be updated to the location of
/// the face found in the rotation found.  NULL if not desired.
///
/// \param[out] aCropLocationRotated A pointer to a Box which will be updated to the location of
/// the cropped photo in the rotation found. Only set if aCropToPhoto = true.
/// NULL if not desired.
///
/// \param[in] aCropToPhoto A boolean indicating whether to crop to an area around the face
/// (photo area) instead of the face found.
///
/// \param[in] aScaleFactor An int specifying how much to downscale the image before face
/// find.  (1 - 10) 1 = none, 2 = half, etc.
///
/// \param[out] aEyeCount A pointer to an int which will be updated to the number of eyes
/// found.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
...
/// handle this task.
///
#if defined ENABLE_GDIPLUS && defined MMM_ENABLE_OPENCV
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_ImageCropToFace(
	Gdiplus::Bitmap *aInputImage,
	Gdiplus::Bitmap **aOutputImage,
	bool aTryAllRotationsNeeded,
	int aRotationsNeeded,
	int *aRotationFound,
	Box *aFaceLocation,
	Box *aCropLocation,
	Box *aFaceLocationRotated,
	Box *aCropLocationRotated,
	bool aCropToPhoto,
	int aScaleFactor,
	int *aEyeCount
);
...
/// Performs Lite data Capture on the document.
///
/// \ingroup ImageProcModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// time and return immediately.
///
/// \param[in] aInputImageInfrared A GDI+ Bitmap object which is the source image infrared to read from.
///
/// \param[in] aInputImageVisible A GDI+ Bitmap object which is the source image visible to read from.
///
/// \param[in] aLineCount - number of mrz lines already recognized (for ICAO docs).
///
/// \param[in] aLineData - pointer to string data of each mrz line.
///
/// \param[in] aDocProcessingSettings A pointer to a DocProcessingSettings structure which should contain the
/// required settings for the \pagereader.
///
/// \param[in] aImageSettings A pointer to a ImageSettings structure which should contain the
/// required settings for the \pagereader.
///
/// \param[in] aOcrSettings A pointer to a OcrSettings structure which should contain the
/// required settings for the \pagereader.
///
/// \param[in] aOcrUserSettings A pointer to a OcrUserSettings structure which should contain the
/// required settings for the \pagereader.
///
/// \param[in] aDataCallback A user-defined #MMMReaderDataCallback function which will receive
/// the codeline that was read as a string.
...
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_ImageLiteDataCapture(
	bool aBlocking,
	Gdiplus::Bitmap *aInputImageInfrared,
	Gdiplus::Bitmap *aInputImageVisible,
	DocProcessingSettings *aDocProcessingSettings,
...
/// Performs Text Data Extract on the document.
///
/// \ingroup ImageProcModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_ImageTextDataExtract(
	bool aBlocking,
	Gdiplus::Bitmap *aInputImageInfrared,
	Gdiplus::Bitmap *aInputImageVisible,
	DocProcessingSettings *aDocProcessingSettings,
...
///
/// \sa MMMReader_ShutdownLogging(), MMMReader_GetLoggingOptions(), MMMReader_SetLoggingOptions(), MMMReader_LL_LogMessage(),
...
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_GetLoggingOptions(
	LoggingSettings* aOptions
);
...
/// \sa MMMReader_ImagePluginLoad(), MMMReaderPluginData
#ifdef ENABLE_GDIPLUS
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_ImagePluginDecodeEx(
	bool aBlocking,
	HANDLE aDecoderHandle,
	Gdiplus::Bitmap *aInputImage,
	char *aDocumentCodeline,
...
/// RFID chip. See the remarks section below for more details.
///
/// \param[in] aApplication Specify the #PASSPORT_APPLICATION to select.
/// See the remarks section below for more details.
///
/// \param[in] aAuxiliaryData Specify the #eIDAuxiliaryData for Auxiliary Data Verification of
/// the eID RF application.  This includes Age, Document and Community ID verification.
///
/// \param[in/out] aRestrictedIdentificationData Specify the #eIDRestrictedIdentificationData for Restricted Identification of
/// the eID RF application.  The SectorPublicKey 1 or 2, encoded per TR 03110 Part 3 B.4.1, should be set to
/// perform Restricted Identification.  If successful #RESTRICTED_IDENTIFICATION_STATUS, the SectorIdentifier1 or 2, respectively, 
/// will be set by the SDK from the chip response data.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
...
/// MMMReader_RFGetReaderCount() API.
///
/// \param[in] aApplication Specify the #PASSPORT_APPLICATION to select, such as ePassport/eID.  Only
/// applies when \a aSelectLDSApplication = true.
///
/// \remarks This API will attempt to open an RFID chip immediately, meaning if the RFID chip
/// is not yet present around the smart card reader device then this API will fail. Consider
/// using the MMMReader_RFWaitForOpen() API instead for such scenarios.
...
/// MMMReader_RFValidateDocSignerCert(), MMMReader_RFGetChipAuthenticationStatus(), 
/// MMMReader_RFGetTerminalAuthenticationStatus()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFOpenEx(
	bool aBlocking,
	char *aBACString,
	ANTENNA_MODE aAntennaMode,
	int aMaxAPDUAttempts,
	...
	eIDAuxiliaryData *aAuxiliaryData,
	eIDRestrictedIdentificationData *aRestrictedIdentificationData
);

/// Selects an application for reading from an already open RFID chip from a given smart card reader.
///
/// \ingroup RfidModule
///
/// \note This API is only relevant if using the \llapi.
...
/// time and return immediately.
///
/// \param[in] aApplicationSelectedCallback A user-defined #MMMReaderEventCallback function to
/// receive notification when the specified application has been successfully selected
...
/// RFID chip. See the remarks section below for more details.
///
/// \param[in] aApplication Specify the #PASSPORT_APPLICATION to select, such as ePassport/eID.
///
/// \param[in] aAuxiliaryData Specify the #eIDAuxiliaryData for Auxiliary Data Verification of
/// the eID RF application.  This includes Age, Document and Community ID verification.
///
...
/// The number of smart card readers available can be determined by using the
/// MMMReader_RFGetReaderCount() API.
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFSelectApplication(
	bool aBlocking,
...
/// MMMReader_RFValidateDocSignerCert(), MMMReader_RFGetChipAuthenticationStatus(),
/// MMMReader_RFGetTerminalAuthenticationStatus(), MMMReader_RFCheckPassiveAuthentication()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFWaitForOpenEx(
	bool aBlocking,
	char *aBACString,
	ANTENNA_MODE aAntennaMode,
	int aMaxAPDUAttempts,
...
/// Determines the status of an #MMMReaderRFStatus item.
///
/// \ingroup RfidModule
///
/// \note This API is only relevant if using the \llapi.
...
/// time and return immediately.
///
/// \param[in] aItem A #MMMReaderRFStatus item to retrieve the result as a #TRISTATE value.
///
/// \param[in] aDataCallback A user-defined #MMMReaderDataCallback function which will receive
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFGetStatus(
	bool aBlocking,
	MMMReaderRFStatus aItem,
	MMMReaderDataCallback aDataCallback,
	void *aParam,
	int aSequenceNum,
	HANDLE aEvent
...
/// time and return immediately.
///
/// \param[in] aSecurityObjectFile Specify which security object data file on the RFID chip to use
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFValidateSignatureEx(
	bool aBlocking,
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFValidateSignedAttributesEx(
	bool aBlocking,
	MMMReaderRFItem aSecurityObjectFile,
	MMMReaderDataCallback aDataCallback,
	void *aParam,
...
/// time and return immediately.
///
/// \param[in] aOnChipCertFile Specify which data file on the RFID chip to load the DSC from
/// for validation. See #MMMReaderRFItem for available files. #RFID_EF_SOD, #RFID_EF_CARD_SECURITY
/// and #RFID_EF_CHIP_SECURITY are acceptable values.
///
/// \param[in] aDataCallback A user-defined #MMMReaderDataCallback function which will receive
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen(), MMMReader_RFSetCertificateCallback()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFValidateDocSignerCertEx(
	bool aBlocking,
...
/// Parses an EF.SOD file and returns the hash values for data groups.
///
/// \ingroup RfidModule
///
/// \note This API is only relevant if using the \llapi.
///
/// \param[in] efSodEncoded A pointer to a binary buffer which contains the encoded EF.SOD file contents, as read from the chip.
///
/// \param[in] efSodLength The size of the \a efSodEncoded buffer.
///
/// \param[out] hashes The address of a pointer to a buffer, which will internally be allocated to 
/// an array of buffers for each DG hash value found up to \a hashesLength.
/// Memory is released internally.
///
///		unsigned char** hashes = NULL;
///		int* hashLengths = NULL;
///		int hashesLength = 0;
///		
///		if (MMMReader_RFGetEfSodHashes(encodedSod, encodedSodSize, &hashes, &hashLengths, &hashesLength) == NO_ERROR_OCCURRED)
///		{
///			// Do something with the hash values
///			unsigned char *dg1Hash = new unsigned char[hashLengths[0]];
///			memcpy(dg1Hash, hashes[0], hashLengths[0]);
///
///			delete[] dg1Hash;
///		}
///
/// \param[out] hashLengths The address of a pointer to a int, which will internally be allocated to 
/// an array of ints for the length of each corresponding DG hash value in \a hashes.
/// Memory is released internally.
///
/// \param[out] hashesLength The address of an integer, which will be set to the length
/// of the hashes and hashLengths buffers after the call.  This is the number of data groups
/// in the EF.SOD, each of which may or may not have a hash value.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
/// \sa MMMReader_RFGetFile()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFGetEfSodHashes(
	unsigned char* efSodEncoded,
	int efSodLength,
...
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_ParseInitialise(ParserSettings* aParserSettings);
...
/// Decodes an EU Digital Green Certificate (DGC) / Digital COVID Certificate (DCC) as read from a barcode.
/// The decoded data includes the Health Certificate Claim (CWT Container) with raw JSON data and parsed
...
/// \note This API is only relevant if using the \llapi.
///
/// \param[in] aData Encoded health data string (PREFIX + BASE45).
///
/// \param[out] aDataDecoded A pointer to a eDV_DigitalGreenCertificateData which is filled with the decoded health certificate data.
///
/// \param[out] aDGCSignatureStatus A pointer to an MMMReaderValidationCode which is filled with the signature status of the DGC.
///             *** Only set if enabled by ParserSettings.puDGCSettings.puDGCValidateSignature ***
/// 
/// \param[out] aDocSignerCertSignatureStatus A pointer to an MMMReaderValidationCode which is filled with the signature status of the
///             Doc Signer Cert used to validate the DGC.  The Doc Signer Cert is validated with the Country Signer Cert (CSC).
///             *** Only set if enabled by ParserSettings.puDGCSettings.puDGCValidateDocSignerCert ***
/// 
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
...
/// MMMReader_ParseInitialise().
///
/// \remarks Optionally also validates the DGC signature and DSC signature based on ParserSettings.puDGCSettings.
/// 
/// \sa MMMReader_ParseInitialise(), 
///     ParserSettings.puDGCSettings
///     ParserSettings.puDGCSettings.puDGCValidateSignature, 
///     ParserSettings.puDGCSettings.puDGCValidateDocSignerCert
/// 
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_DecodeDigitalGreenCertificate(
	const char* aData,
	eDV_DigitalGreenCertificateData* aDecodedData,
	MMMReaderValidationCode* aDGCSignatureStatus,
	MMMReaderValidationCode* aDocSignerCertSignatureStatus
);
...
/// \return TRUE if success
//////
MMM_READER_LOW_LEVEL_API bool MMMReader_LL_LockReader();
...
/// \note This API is only relevant if using the \llapi.
///
/// \param[in] aSACStatus A pointer to the SAC status to be set as a #TRISTATE value.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFGetSACStatusLite(
	TRISTATE *aSACStatus
);
...
/// \note This API is only relevant if using the \llapi.
///
/// \param[in] aBACStatus A pointer to the BAC status to be set as a #TRISTATE value.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
...
///
/// \sa MMMReader_RFOpen(), MMMReader_RFWaitForOpen()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_RFGetBACStatusLite(
	TRISTATE *aBACStatus
);

/// Retrieves and returns the camera's license data from its config file.
///
/// \note This API is only relevant if using the \llapi.
///
/// \return TRUE if success
//////
MMM_READER_LOW_LEVEL_API bool MMMReader_CameraLicenseData(char *aData, unsigned long *aDataLen);
/// \endcond
...
/// Initialises the \pagereader IDV Remote interface
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
///
/// \param[in] aReaderDir Specify a directory for debug files to be written.
///
/// \return #NO_ERROR_OCCURRED if successful, otherwise a #MMMReaderErrorCode stating what
/// error occurred.
///
/// \attention The MMMReader_IDVRemoteShutdown() API must be called when finished with the IDV remote interface
/// to free all resources allocated.
///
/// \sa MMMReader_IDVRemoteShutdown(), MMMReader_IsIDVRemoteInitialised(), MMMReader_IDVRemoteResume(),
/// MMMReader_IDVRemoteSuspend(), MMMReader_IsIDVRemoteSuspended()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteInitialise(RTCHAR *aReaderDir);

/// Shuts down the \pagereader IDV remote interface.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// the camera to a consistent state for other applications to use.
///
/// \sa MMMReader_IDVRemoteInitialise(), MMMReader_IsIDVRemoteInitialised()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteShutdown();

/// Determines whether any operation is currently active in the IDV remote interface module.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// \c false otherwise.
///
/// \sa MMMReader_IDVRemoteCancelQueue()
MMM_READER_LOW_LEVEL_API bool MMMReader_IsIDVRemoteActive();

/// Cancels any pending IDV remote interface operations that have been queued by non-blocking API calls
/// within the IDV remote interface module, and attempts to cancel any currently executing operation.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// \remarks This API will return as soon as it has indicated that the operations are to be
/// cancelled, not when any currently in-progress operations are complete. Therefore, if the
/// host application needs to wait for completion, the MMMReader_IsIDVRemoteActive() API
/// can be used to check for completion.
///
/// \sa MMMReader_IsIDVRemoteActive()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteCancelQueue();

/// Determines whether the \pagereader IDV remote interface has been initialised.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
///
/// \return \c true if MMMReader_IDVRemoteInitialise() has been called, or \c false otherwise.
///
/// \sa MMMReader_IDVRemoteInitialise()
MMM_READER_LOW_LEVEL_API bool MMMReader_IsIDVRemoteInitialised();


/// Performs a single document verification to IDV server.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// time and return immediately.
///
/// \param[in] aInputImageInfrared A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageVisible A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageUltraviolet A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageInfraredRear A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageVisibleRear A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageUltravioletRear A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aLineCount A count of MRZ line being passed in the aLineData param.
///
/// \param[in] aLineData An array of mrz lines.
///
/// \param[in] aTimeout A maxinum time in second to wait for a reply from the IDV verification server.
///
/// \param[in] aDocProcessingSettings A pointer to a DocProcessingSettings structure which should contain the
/// required settings for the \pagereader.
///
/// \param[in] aAccessToken A pointer to a stringified token that will be used instead of the server authorization call. 
/// Default to NULL if not used.
///
/// \param[in] aDataCallback A user-defined #MMMReaderDataCallback function which will receive
/// the codeline that was read as a string.
///
...
///
#ifdef ENABLE_GDIPLUS
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteDocumentVerify(
	bool aBlocking,
	Gdiplus::Bitmap *aInputImageInfrared,
	Gdiplus::Bitmap *aInputImageVisible,
	Gdiplus::Bitmap *aInputImageUltraviolet,
	Gdiplus::Bitmap *aInputImageInfraredRear,
	Gdiplus::Bitmap *aInputImageVisibleRear,
	Gdiplus::Bitmap *aInputImageUltravioletRear,
	int aLineCount, char *aLineData[],
...
/// Performs a single chip verification to IDV server.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// time and return immediately.
///
/// \param[in] aDG01 A pointer to a buffer containing the complete DG01 datagroup.
///
/// \param[in] aDG01Length A length for the aDG01 buffer.
///
/// \param[in] aDG02 A pointer to a buffer containing the complete DG02 datagroup.
///
/// \param[in] aDG02Length A length for the aDG02 buffer.
///
/// \param[in] aDG03 A pointer to a buffer containing the complete DG03 datagroup.
///
/// \param[in] aDG03Length A length for the aDG03 buffer.
///
/// \param[in] aEFCOM A pointer to a buffer containing the complete EFCOM datagroup.
///
/// \param[in] aEFCOMLength A length for the aEFCOM buffer.
///
/// \param[in] aSOD A pointer to a buffer containing the complete SOD datagroup.
///
/// \param[in] aSODLength A length for the aSOD buffer.
///
/// \param[in] aTimeout A maxinum time in second to wait for a reply from the IDV verification server.
///
/// \param[in] aDocProcessingSettings A pointer to a DocProcessingSettings structure which should contain the
...
/// error occurred.
///
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteChipVerify(
	bool aBlocking,
	unsigned char *aDG01, unsigned long aDG01Length,
	unsigned char *aDG02, unsigned long aDG02Length,
	unsigned char *aDG03, unsigned long aDG03Length,
	unsigned char *aEFCOM, unsigned long aEFCOMLength,
	unsigned char *aSOD, unsigned long aSODLength,
	unsigned long aTimeout,
	DocProcessingSettings *aDocProcessingSettings,
	char *aAccessToken,
	MMMReaderDataCallback aDataCallback,
...
/// Performs a single face match to IDV server.
///
/// \ingroup IDVRemoteModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// time and return immediately.
///
/// \param[in] aInputImageVisiblePageFace A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aInputImageVisibleChipFace A GDI+ Bitmap object which is the source image to read from.
///
/// \param[in] aTimeout A maxinum time in second to wait for a reply from the IDV verification server.
///
/// \param[in] aDocProcessingSettings A pointer to a DocProcessingSettings structure which should contain the
...
///
#ifdef ENABLE_GDIPLUS
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_IDVRemoteFaceMatch(
	bool aBlocking,
	Gdiplus::Bitmap *aInputImageVisiblePageFace,
	Gdiplus::Bitmap *aInputImageVisibleChipFace,
	unsigned long aTimeout,
	DocProcessingSettings *aDocProcessingSettings,
	char *aAccessToken,
	MMMReaderDataCallback aDataCallback,
...
/// Initialises the FaceMatch module.
///
/// \ingroup FaceMatchModule
///
/// \note This API is only relevant if using the \llapi.
///
/// \sa MMMReader_FaceMatchShutdown()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_FaceMatchInitialise();


/// Shuts down the FaceMatch module and frees all related resources.
///
/// \ingroup FaceMatchModule
///
/// \note This API is only relevant if using the \llapi.
...
/// error occurred.
///
/// \sa MMMReader_FaceMatchInitialise()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_FaceMatchShutdown();


/// Determines whether the FaceMatch module has been initialised.
///
/// \ingroupFaceMatchModule
///
/// \note This API is only relevant if using the \llapi.
///
/// \return \c true if MMMReader_FaceMatchInitialise() has been called, or \c false otherwise.
///
/// \sa MMMReader_FaceMatchInitialise();
MMM_READER_LOW_LEVEL_API bool MMMReader_IsFaceMatchInitialised();
...
/// error occurred.
///
/// \sa MMMReader_FaceMatchInitialise(), MMMReader_FaceMatchShutdown()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_CogentFaceMatch(void* image1, void* image2, int &score, int &matchResult);

/// \endcond


/////////// LiteDataCapture APIs

/// \cond PAGESDK

/// Initialises the \sdk LiteDataCapture module and allocates resources required for image
/// processing.
///
/// \ingroup LiteDataCaptureModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
///
/// \param[in] aReaderDir A null-terminated string containing the directory path to the
...
/// how to proceed.
///
/// \sa MMMReader_LiteDataCaptureShutdown(), MMMReader_IsLiteDataCaptureInitialised()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_LiteDataCaptureInitialise(
	RTCHAR *aReaderDir
);

/// Shuts down the \sdk LiteDataCapture module and frees all related resources.
///
/// \ingroup LiteDataCaptureModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
...
/// error occurred.
///
/// \sa MMMReader_LiteDataCaptureInitialise(), MMMReader_IsLiteDataCaptureInitialised()
MMM_READER_LOW_LEVEL_API MMMReaderErrorCode MMMReader_LiteDataCaptureShutdown();

/// Determines whether the \sdk LiteDataCapture module has been initialised.
///
/// \ingroup LiteDataCaptureModule
///
/// \note This API is only relevant if using a \pagereader with the \llapi.
///
/// \return \c true if MMMReader_LiteDataCaptureInitialise() has been called, or \c false
/// otherwise.
///
/// \sa MMMReader_LiteDataCaptureInitialise()
MMM_READER_LOW_LEVEL_API bool MMMReader_IsLiteDataCaptureInitialised();
